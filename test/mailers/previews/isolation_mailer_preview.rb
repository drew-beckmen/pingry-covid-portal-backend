...
class IsolationMailerPreview < ActionMailer::Preview
    ...
    def isolation_started_email_student
        isolation = Isolation.new(start_isolation: Date.today, confirmed: true, completed: false, student_id: 1565)
    ...
    def isolation_started_email_adult 
        isolation = Isolation.new(start_isolation: Date.today, confirmed: true, end_date: Date.today + 10, completed: false, student_id: 1565)
        currentStudent = Student.find(isolation.student_id)
    ...
    def isolation_updated_email_student
        isolation = Isolation.new(start_isolation: Date.today - 10, confirmed: true, date_improving: Date.today - 10, end_date: Date.today, completed: true, student_id: 1565)
        currentStudent = Student.find(isolation.student_id)
    ...
    def potential_isolation_started_email_adult
        isolation = Isolation.new(start_isolation: Date.today - 10, barcode: "ABCDEF", potential: true, date_improving: Date.today - 10, end_date: Date.today, completed: true, student_id: 1565)
        currentStudent = Student.find(isolation.student_id)
